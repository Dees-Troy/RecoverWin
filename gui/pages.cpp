...
PageSet* PageManager::mCurrentSet = NULL;
...
int PageSet::SetPage(std::string page)
{
    ...
    if (tmp)
    {
        ...
        DataManager::SetValue(VAR_CURRENT_PAGE, page);
        return 0;
    }
    else
    {
...
PageSet* PageManager::SelectPackage(std::string name)
{
    ...
    {
        mCurrentSet = tmp;
        DataManager::SetValue(VAR_CURRENT_PACKAGE, name);
    }
    else
...
bool PageManager::IsCurrentPackage(std::string name)
{
    return (FindPackage(name) == mCurrentSet);
}
