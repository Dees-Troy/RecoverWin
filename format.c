...
static int phx_format_rmfr(const char* device)
{
    char cmd[256];
...
static int phx_format_vfat(const char* device)
{
    char exe[512];
...
static int phx_format_ext23(const char* fstype, const char* device)
{
...
static int phx_format_ext4(const char* device)
{
    return phx_format_ext23("ext4", device);
}

int phx_format(const char *fstype, const char *fsblock)
{
    int result = -1;
    ...
    if (!file_exists(fsblock))
    {
        if (strcmp(fsblock, "userdata") == 0 && strcmp(fstype, "yaffs2") == 0) {
			...
			// there is handling in phx_format_mtd if the fsblock doesn't exist, so we'll just keep going.
		} else {
    ...
    if (strcmp(fstype, "yaffs2") == 0 && v && strcmp(v->mount_point, "/efs") == 0)
        result = phx_format_rmfr(fsblock);

    else if (strcmp(fstype, "yaffs2") == 0 || strcmp(fstype, "mtd") == 0)
        result = phx_format_mtd(fsblock);

    else if (strcmp(fstype, "ext4") == 0)
        result = phx_format_ext4(fsblock);

    else if (strcmp(fstype, "vfat") == 0)
        result = phx_format_vfat(fsblock);

    else if (memcmp(fstype, "ext", 3) == 0)
        result = phx_format_ext23(fstype, fsblock);
